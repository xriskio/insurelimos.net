...
function StateSection({ name, abbr, cities, bgClass = "bg-white" }: { name: string; abbr: string; cities: CityLocation[]; bgClass?: string }) {
  ...
  return (
    <section className={`py-12 ${bgClass}`} aria-labelledby={sectionId}>
      <div className="container mx-auto px-4">
        <h2 id={sectionId} className="text-2xl md:text-3xl font-bold text-primary mb-4">{name}</h2>
        <p className="text-muted-foreground mb-6 max-w-3xl">
          Serving transportation businesses throughout {name} with comprehensive commercial auto insurance solutions for NEMT providers, paratransit operators, bus companies, ambulance services, and limousine fleets.
        </p>
...
export default function Locations() {
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <Seo 
        title="Transportation Insurance by Location - 18 US States"
        description="Find specialized transportation insurance for NEMT, paratransit, buses, ambulances, limousines, and taxis across 18 US states. CA, AZ, CO, ID, IL, KS, KY, MN, MO, NV, OH, OK, PA, TN, TX, UT, VA, WI."
      ...
      <main className="flex-1" id="main-content" role="main">
        <section className="bg-primary text-white py-16 md:py-24" aria-labelledby="locations-heading">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-xl text-primary-foreground/80 max-w-3xl mx-auto mb-8">
              Find specialized commercial transportation insurance tailored to your city. We provide coverage for NEMT, paratransit, buses, ambulances, limousines, and taxis across 18 US states.
            </p>
            <div className="flex flex-wrap justify-center gap-2">
              {STATE_DATA.map((state) => (
                <a 
                  key={state.abbr} 
                  href={`#${state.abbr.toLowerCase()}-locations`}
        ...
        {STATE_DATA.map((state, index) => (
          <StateSection 
            ...
            abbr={state.abbr}
            cities={state.cities}
            bgClass={index % 2 === 0 ? "bg-secondary/20" : "bg-white"}
          />
        ))}

        <section className="py-16 bg-muted" aria-labelledby="insurance-types">
          <div className="container mx-auto px-4">
